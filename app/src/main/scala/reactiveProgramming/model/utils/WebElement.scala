...
object WebElement:
  case class WebElementImpl(private val url: String, private val word: String, private val depth: Int):
    def connect(): (Int, java.util.List[String]) =
      val doc = Jsoup.connect(url).get()
      val word = doc.html().trim.replaceAll("<[^>]*>", "").lines().map(_.toLowerCase).filter(_.contains("wikipedia")).count().toInt
